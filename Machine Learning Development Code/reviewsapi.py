...
Original file is located at
    https://colab.research.google.com/drive/1mvl5ekAwC8GMlE7f38kkCSg2J8D4FGCl
"""
...
"""# **<h1>Covertig API Data into Dataframe**"""
...
def jsonToDataframe(df1): 
  ...
  for business in df1['data']:
    ...
    for review in business['reviews']:
      ...
      df2.append({'name': obj['name'],'google_ratings':obj['google_ratings'],'reviews':obj['reviews']})
  ...
  for index,val in dataframe.iterrows():
    ...
    for v in val["reviews"]:
      df=df.append({"name":val["name"],"reviews":v,"google_rating":val['google_ratings']},ignore_index=True)
  return df
...
def systemRating(df):
  ...
  for val in df["name"].unique():
    ...
    final=final.append({"name":val,"google_rating":df2["google_rating"].iloc[0],"system_rating":ratings},ignore_index=True)
  return final
...
def getLocations(lat,lng,category,actualLat,actualLng):
  ...
  api_key='y3s9ZkkmoGS-H-lv4qMC8gZCWAPun7GEn8tiEzL8EiTR9_3EYYQuOWmzKTjbiQsDnfmvHOZL-MX1lj-kkvnK0oqLWdIWKYXftJSyFC_bVDIpT3ZkxvzOoRmtsmluYXYx'
  headers = {'Authorization': 'Bearer {}'.format(api_key)}
  search_api_url = 'https://api.yelp.com/v3/businesses/search?categories=%s&limit=7&latitude=%f&longitude=%f&radius=1000'%(category,lat,lng)
...
def basic():
  ...
  try:
    client = MongoClient("mongodb+srv://haseeb:123abcd@cluster0.02mci.mongodb.net/Cleverus?retryWrites=true&w=majority")
...
def dataStore():
  ...
  if(content==None or businessType==None):
    ...
    #ye content type bhi ignore kr daita 
    #safty ky liye likha hai.
    #Ye na chaley to content=request.json() ye chalaa daie.
...
def getRandomResturants():
  try:
    ...
    # data_to_send=[]
    # if(len(resturants)>=3 and len(hotels)>=3):
    #   data_to_send={"resturants":resturants[1:4],"hotels":hotels[1:4]}
    # elif(len(resturants)>=3 and len(hotels)<3):
    #   data_to_send={"resturants":resturants[1:4],"hotels":hotels}
    # elif(len(resturants)<3 and len(hotels)>=3):
    #   data_to_send={"resturants":resturants,"hotels":hotels[1:4]}
    # else:
    data_to_send={"resturants":resturants[1:4],"hotels":hotels[1:4]}
